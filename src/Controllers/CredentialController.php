 ...
 * Every issued credential is assigned a standardised, human-readable serial
 * number at issuance time. This is the Malaysian-style identifier printed on
 * the physical/PDF certificate and is what Verifiers use to look up a
 * credential — replacing the old urn:uuid lookup.
 *
 *   Format : KZ-<YEAR>-<8 uppercase hex chars>
 *   Example: KZ-2024-1A2B3C4D
 *
...
final class CredentialController extends Controller
{
    ...
    public function issue(array $params = []): void
    {
        ...
        // ── AUTO-GENERATE the standardised Certificate Serial Number ──────────
    ...
    public function verify(array $params = []): void
    {
        ...
        $certificateSerial = strtoupper(trim(
        ...
        foreach ($allRows as $row) {
            ...
            if ($serialInRow !== null && strtoupper($serialInRow) === $certificateSerial) {
                $cred = $row;
                break;
            }
        }
    ...
    /**
     * Generate a standardised, unique Malaysian-style certificate serial.
     *
     * Format: KZ-<YEAR>-<8 uppercase hex chars>   e.g. KZ-2024-1A2B3C4D
     *
     * Re-rolls on the rare chance of a collision against an existing
     * credential's stored subject/jsonld.
     */
    private function generateCertificateSerial(): string
    {
        ...
        for ($attempt = 0; $attempt < 10; $attempt++) {
            $random = strtoupper(bin2hex(random_bytes(4))); // 8 hex chars
            $serial = "KZ-{$year}-{$random}";
        ...
        return "KZ-{$year}-" . strtoupper(bin2hex(random_bytes(6)));
    }
    ...
    private function serialExists(string $serial): bool
    {
        try {
            ...
            foreach (($stmt->fetchAll() ?: []) as $row) {
                ...
                if ($existing !== null && strtoupper($existing) === strtoupper($serial)) {
                    return true;
                }
            }
        } catch (\Throwable $e) {
